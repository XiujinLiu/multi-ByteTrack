...
static void decode_outputs(float* prob, vector<Object>& objects, float scale, const int img_w, const int img_h) {
    // 最最开始的objects是空集
        ...
        // proposals: pass by ref to get value
...
int main(int argc, char** argv) {
	...
	while (true)
    {
        ...
        cout << 1 << endl;
        vector<STrack> output_stracks = tracker.update(obikojects);
        cout << 2<< endl;
		...
		{
            int classnum = output_stracks[i].classid;
			vector<float> tlwh = output_stracks[i].tlwh;
			bool vertical = tlwh[2] / tlwh[3] > 1.6;
			if (tlwh[2] * tlwh[3] > 20 && !vertical)
			{
				Scalar s = tracker.get_color(output_stracks[i].track_id);
				putText(img, format("%d", output_stracks[i].track_id), Point(tlwh[0], tlwh[1] - 20), 
                        0, 0.6, Scalar(0, 0, 255), 2, LINE_AA);
                putText(img, format("%d", classnum), Point(tlwh[0], tlwh[1] - 5),
                        0, 0.6, Scalar(0, 0, 255), 2, LINE_AA);
